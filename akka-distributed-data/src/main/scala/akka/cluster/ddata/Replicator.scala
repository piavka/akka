 ...
 *   The memember must have all given roles. All members are used if empty.
...
final class Replicator(settings: ReplicatorSettings) extends Actor with ActorLogging {
  ...
  require(
    roles.forall(cluster.selfRoles.contains),
  ...
  override def preStart(): Unit = {
    ...
    // not using LeaderChanged/RoleLeaderChanged because here we need one node indepentent of team
  ...
  def matchingRole(m: Member): Boolean = roles.forall(m.hasRole)
